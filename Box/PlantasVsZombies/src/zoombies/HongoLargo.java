...
public class HongoLargo {
    ...
    HongoLargo(Plantas p,int x,int y){
        this.miedo = false;
        this.bandera = false;
        this.p=p;
    ...
    public void generarBala(){
        for(zombies zombie:p.vectorZombies){
            if(zombie.y>=this.y&&zombie.y<=this.y+p.pixel/2&&zombie.x<p.extraxIzq+(p.pixel*8)&&zombie.x>this.x+p.pixel/2 && !miedo){
             //   contador2++;
    ...
    public void draw(Graphics2D g2){
        ...
        generarBala();
        colision();
        g2.drawImage(p.plantas[10], x,y,p.pixel,p.pixel,p);
    ...
    public void colision(){
        ...
        for(zombies zombie:p.vectorZombies){
            if((this.x)+p.pixel>=zombie.x && (this.x)+p.pixel<=(zombie.x+p.pixel)*2 && this.y>=zombie.y && this.y<=zombie.y+p.pixel/2){
               miedo = true;
            }else if(!((this.x)+p.pixel>=zombie.x && (this.x)+p.pixel<=(zombie.x+p.pixel)*2 && this.y>=zombie.y && this.y<=zombie.y+p.pixel/2)){
                miedo = false;
            }
        }
    }
