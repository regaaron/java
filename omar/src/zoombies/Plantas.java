...
public class Plantas extends JComponent implements Runnable {
    ...
    int FPS = 30;
    long targetTime = 1000 / FPS; // Calcula el tiempo objetivo por cuadro en milisegundos
    int cargas = 0;
    int puntos = 3000; // puntos iniciales
    Thread gameThread;
    BufferedImage[] plantas = new BufferedImage[15];
    BufferedImage back,dia,noche, niebla, score, girasol, nuez, gisante1, pala1, pala2, explosion, gameover, doblegizante,
            triplegizante; // imagenes
    ...
    Nivel1 lvl1; // objeto que es el nivel o que va ir creadno zombies en tiempo y cantidad
                 // especificados
    ...
    Plantas(Carta[] mazoplantas,int nivel,int ecenario, boolean bandera) {
        ...
        if(nivel==1){
              lvl1 = new Nivel1(this,bandera); // inicializamos el objeto lvl1 y le pasamos este
        ...
        switch(ecenario){
            case 1: back=dia; break;
            case 2: back=noche; break;
            case 3: back=noche; boolniebla=true; break;
            default: System.out.println("Error en escenarios");
        }
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evento) {
                ...
                if (evento.getX() > 59 && evento.getX() < 133 && evento.getY() > 25 && evento.getY() < 87) {
                    ...
                    // ya que si no ala hora de colocarlo y si al mismo tiempo teniamos la pala lo
                    // agrega
                ...
                if (evento.getX() > 392 && evento.getX() < 453 && evento.getY() > 19 && evento.getY() < 61) {
                    // si esta activa desactivamos si no activamos y colocamos las equitenas el
                    // false
                    if (bpala) {
                        bpala = false;
                    } else {
    ...
    // este es el metodo que teniamos arriba de colocar en la matriz dependiendo del
    // tag activado
    public void ColocarMAtriz(int i, int j) {
        // antes de eco checamos que no este una planta ya en esa posicion
        if (matriz[j][i] == 0) {
            // si es el tag 1 colocammos 1 colocamos el tag en falso para no poder plantar
            // mas
    ...
    public void agregar(int option, int i, int j) {
        switch (option) {
            case 0:
                vectorGisantes.add(new Cactus(this, i, j));
                break;
            case 1:
                vectorGisantes.add(new cereza(this, i, j));
                break;// cereza
            case 2:
                vectorGisantes.add(new gisanteHielo(this, i, j));
                break;// chile
            case 3:
                vectorGisantes.add(new Congelar(this, i, j));
                break;// congela
            case 4:
                vectorGisantes.add(new Girazol(this, i, j));
                break;
            case 5:
                vectorGisantes.add(new Gizantes(this, i, j));
                break;
            case 6:
                vectorGisantes.add(new DobleGizantes(this, i, j));
                break;
            case 7:
                vectorGisantes.add(new TripleGizantes(this, i, j));
                break;
            case 8:
                vectorGisantes.add(new gisanteHielo(this, i, j));
                break;// snow
            case 9:
                vectorGisantes.add(new mini(this, i, j));
                break;// hongopequeño
            case 10:
                vectorhongolargo.add(new HongoLargo(this, i, j));
                break;
            case 11:
                vectorhongonoche.add(new HongoNoche(this, i, j));
                break;
            case 12:
                vectorGisantes.add(new Mina(this, i, j));
                break;
            case 13:
                vectorGisantes.add(new Nuez(this, i, j));
                break;
            case 14:
                vectorGisantes.add(new NuezGrande(this, i, j));
                break;
        }
        matriz[j][i] = 1;
    ...
    public void paint(Graphics g) {
        ...
        if (bpala) {
            g.drawImage(pala2, posx - pixel / 2, posy - pixel / 2, pixel - pixel / 4, pixel - pixel / 4, null);
        }
        ...
        if (over) {
            g2.drawImage(gameover, ((screenX / 2) - (pixel) - tiempo), (screenY / 2) - tiempo, pixel + (tiempo * 2),
                    pixel + (2 * tiempo), null);
        }
    ...
    private void cargarImagenes() {
        try {
            ...
            dia = ImageIO.read(getClass().getResourceAsStream("/Java/resources/background.png"));
            noche = ImageIO.read(getClass().getResourceAsStream("/Java/resources/background2.png"));
    ...
    // logica de los fps no se explicarlo muy bien pero revisalo ya que ahi
    // actualizamos
    // un par de cosas
    @Override
    public void run() {
        // final int FPS = 35;
        // final long FRAME_TIME = 1000000000L / FPS; // Tiempo en nanosegundos por
        // fotograma
        ...
        // // Aquí va la actualización que deseas realizar a 35 FPS
        ...
        while (true) {
            ...
            repaint();
            try {
                Thread.sleep(targetTime); // Establece el retraso entre cuadros
